...
function mode2 {
  ...
  #git clone https://github.com/TheWalker0/startbs && chmod +x start.sh &&
  mkdir "$DIR" && cd "$DIR" && wget https://minecraft.azureedge.net/bin-linux/bedrock-server-1.8.1.2.zip && unzip bedrock-server-1.8.1.2.zip && rm -r bedrock-server-1.8.1.2.zip && screen -d -m -S Vanilla ./start.sh
}
...
function main {
  ...
  if [ "$MODE" == "1" ]; then
    ...
    apt-get update && apt-get upgrade -y && mkdir BedRock_Server_Software && cd BedRock_Server_Software && wget https://minecraft.azureedge.net/bin-linux/bedrock-server-1.8.1.2.zip && unzip bedrock-server-1.8.1.2.zip && rm -r bedrock-server-1.8.1.2.zip && apt install screen -y  && git clone https://github.com/TheWalker0/startbs && chmod +x start.sh && screen -d -m -S ./start.sh
  elif [ "$MODE" == "2" ]; then
  	mode2
  else
    # exit
